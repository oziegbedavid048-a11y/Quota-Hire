...
import {
  ...
  Sparkles, ExternalLink, CheckCircle, User, Briefcase,
...
export function GenerateCVModal({ isOpen, onClose }: GenerateCVModalProps) {
  ...
  const handleOpenInNewTab = () => {
    if (currentUrlRef.current) window.open(currentUrlRef.current, '_blank', 'noopener,noreferrer');
  };
                            ...
                            >
                              ...
                              {!blobLoading && !blobError && url && (
                                <object
                                  ...
                                  aria-label="Generated CV Preview"
                                >
                                  <div className="absolute inset-0 flex flex-col items-center justify-center bg-gray-50 p-6 text-center">
                                    <FileText className="w-12 h-12 text-gray-300 mb-4" />
                                    <p className="text-gray-600 font-semibold text-sm mb-1">Preview not supported in this browser</p>
                                    <button
                                      onClick={handleOpenInNewTab}
                                      className="flex items-center gap-2 px-4 py-2 bg-blue-900 text-white rounded-xl text-sm font-semibold hover:bg-blue-800 transition mt-3"
                                    >
