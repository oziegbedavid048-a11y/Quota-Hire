...
const s = StyleSheet.create({
  ...
  photoBox: { width: 85, height: 105, borderRadius: 52, overflow: 'hidden' },
  photoImage: { width: '100%', height: '100%', objectFit: 'cover' },
  photoPlaceholder: { width: '100%', height: '100%', backgroundColor: '#d1d5db', alignItems: 'center', justifyContent: 'center' },
  photoPlaceholderText: { fontSize: 28, fontFamily: 'Helvetica-Bold', color: '#6b7280' },
  logoContainer: { alignItems: 'flex-end', justifyContent: 'flex-start' },
  logoImage: { width: 150, height: 50, objectFit: 'contain' },
        ...
        <View style={s.topRow}>
          <View style={s.photoBox}>
            {data.passportImageUrl ? (
              ...
              <View style={s.photoPlaceholder}><Text style={s.photoPlaceholderText}>{initial}</Text></View>
            )}
          </View>
        ...
        {data.workExperience && data.workExperience.length > 0 ? (
          <View>
            ...
            {data.workExperience.filter((e) => e.role || e.employer || e.duties).map((exp, i) => {
              ...
              const titleLine = [exp.role, exp.employer].filter(Boolean).join(' - ');
              return (
